(function($) {
	...
	function headerStyle() {
		if ($('.main-header').length) {
			...
			var logoWhite = document.querySelector('meta[name="logoWhite"]').getAttribute('content');
			var logoBlack = document.querySelector('meta[name="logoBlack"]').getAttribute('content');
			if (windowpos > 250) {
				logoHeader.attr('src', logoBlack);
	...
	// Kota Carousel
	if ($('.kota-carousel').length) {
		$('.kota-carousel').owlCarousel({
			loop: false,
			margin: 20,
			nav: true,
			smartSpeed: 1000,
	...
	if ($('.accordion-box').length) {
		$(".accordion-box").on('click', '.accord-btn', function(e) {
			e.preventDefault();
			...
			if ($accordion.hasClass('active')) {
				$accordion.removeClass('active');
				$accordion.find('.accord-content').slideUp(500);
			} else {
				$accordion.addClass('active');
				$accordion.find('.accord-content').slideDown(500);
			}
		});
	}
	...
	if ($('#donate-popup').length) {
		...
		$('.donate-box-btn').on('click', function(e) {
			...
			var riwayatJabatan = $(this).data('riwayat-jabatan');
			var riwayatPendidikan = $(this).data('riwayat-pendidikan');
			var img = $(this).data('img');
			var mapSrc = 'https://maps.google.com/maps?&hl=en&q=' + encodeURIComponent(nama) + '&t=&z=15&ie=UTF8&iwloc=B&output=embed';

			$('#donate-popup .image img').attr('src', img);
			$('#donate-popup .nama').text(nama);
			$('#donate-popup .jabatan').text(jabatan);
			$('#donate-popup .pangkat').text(pangkat);
			$('#donate-popup .nip').text(nip);
			$('#donate-popup .riwayat-jabatan').html(riwayatJabatan);
			$('#donate-popup .riwayat-pendidikan').html(riwayatPendidikan);
			$('#donate-popup iframe').attr('src', mapSrc);
		});
